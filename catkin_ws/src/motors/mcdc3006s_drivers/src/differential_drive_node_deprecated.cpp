...
private:
    ...
    const char *l_sem_ = "/tmp/sem_l\0";
    const char *r_sem_ = "/tmp/sem_r\0";
...
DiffDriveNode::DiffDriveNode(){
    ...
    ros::param::param<std::string>("~serial_lDevice", serial_l_, "/dev/walker_motor_left\0");
    ros::param::param<std::string>("~serial_rDevice", serial_r_, "/dev/walker_motor_right\0");
...
void DiffDriveNode::timer_cb(const ros::TimerEvent& event) {
    ...
    if (motor_r.move_vel(-(int)rpm_r) != ERR_NOERR)              // notice robot setup
        fprintf(stderr, "There has been an error.\n\r");
    if (motor_l.move_vel((int)rpm_l) != ERR_NOERR)
        fprintf(stderr, "There has been an error.\n\r");

    long int real_rpm_l, real_rpm_r; 
    if (motor_l.get_instant_pos(&real_rpm_l) != ERR_NOERR)
        fprintf(stderr, "There has been an error.\n\r");
    if (motor_l.get_instant_pos(&real_rpm_r) != ERR_NOERR)
        fprintf(stderr, "There has been an error.\n\r");
    ...
    // cout << "real_l: " << real_rpm_l << ", real_r: " << real_rpm_r << endl;
    printf("%ld, %ld\n", real_rpm_l, real_rpm_r);
