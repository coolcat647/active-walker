...
rospy.init_node('passing_wall_4', anonymous=False)
...
tree_ = etree.parse(actor_pkg_path+'/worlds/'+world_name, parser)
...
comment = etree.Comment(' === Start adding the pedestrian === ')
world_.append(comment)
...
comment = etree.Comment(' === Stop adding the pedestrian === ')
world_.append(comment)

# print(etree.tostring(tree_, pretty_print=True))
tree_.write(actor_pkg_path+'/worlds/ped_world.world', pretty_print=True, xml_declaration=True, encoding="utf-8")
rospy.logwarn("passing_wall_4 ok!")
